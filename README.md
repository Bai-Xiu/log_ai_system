...
- Supports import and management of CSV-formatted log files
...
- Python 3.8+
- Dependent libraries: `PyQt5`, `pandas`, `numpy`, `openai`
...
- Choose the CSV log files to analyze from the list
...
- Ensure log files are in CSV format with supported encodings (utf-8, gbk, gb2312, ansi)
...
1. **Q: Unable to load files?**  
   A: Check if the file is in CSV format, if the path contains special characters, or try changing the file encoding
